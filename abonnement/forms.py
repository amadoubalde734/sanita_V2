...
class LicenseForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not duration_years:
            self.add_error("duration_years", "Veuillez sélectionner une durée.")

        if not plan:
            self.add_error("plan", "Veuillez sélectionner une formule.")
        elif not plan.active:
            self.add_error("plan", "Ce plan n'est plus actif.")
...
class PlanForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "name": forms.TextInput(attrs={"class": "form-control", "placeholder": "Exemple : Standard"}),
            "description": forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "Description du plan..."}),
            "price": forms.NumberInput(attrs={"class": "form-control", "min": "0", "step": "0.01", "placeholder": "0"}),
            "currency": forms.TextInput(attrs={"class": "form-control", "value": "GNF", "placeholder": "GNF"}),
            "max_users": forms.NumberInput(attrs={"class": "form-control", "min": "1", "placeholder": "Exemple : 10"}),
            "max_sites": forms.NumberInput(attrs={"class": "form-control", "min": "1", "placeholder": "Exemple : 1"}),
            "unlimited_users": forms.CheckboxInput(attrs={"class": "form-check-input"}),
            "unlimited_sites": forms.CheckboxInput(attrs={"class": "form-check-input"}),
    ...
    def clean_code(self):
        ...
        if not code:
            raise forms.ValidationError("Veuillez sélectionner un code de plan.")
        queryset = Plan.objects.filter(code=code)
        if self.instance.pk:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise forms.ValidationError("Ce code de plan existe déjà.")
        return code

    def clean_name(self):
        name = self.cleaned_data.get("name")
        if not name:
            raise forms.ValidationError("Le nom du plan est obligatoire.")
        name = name.strip()
        queryset = Plan.objects.filter(name__iexact=name)
        if self.instance.pk:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise forms.ValidationError("Un plan portant ce nom existe déjà.")
        return name
    ...
    def clean_currency(self):
        ...
        if not currency:
            raise forms.ValidationError("La devise est obligatoire.")
    ...
    def clean(self):
        ...
        if not unlimited_users:
            if max_users is None:
                self.add_error("max_users", "Veuillez indiquer le nombre maximum d'utilisateurs.")
            elif max_users < 1:
                self.add_error("max_users", "Le nombre d'utilisateurs doit être supérieur à 0.")

        if not unlimited_sites:
            if max_sites is None:
                self.add_error("max_sites", "Veuillez indiquer le nombre maximum de sites.")
            elif max_sites < 1:
                self.add_error("max_sites", "Le nombre de sites doit être supérieur à 0.")
...
class MaintenanceContractForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "amount": forms.NumberInput(attrs={"class": "form-control", "min": "0", "step": "0.01", "placeholder": "Montant du contrat"}),
            "currency": forms.TextInput(attrs={"class": "form-control", "placeholder": "GNF"}),
            "status": forms.Select(attrs={"class": "form-select"}),
            "description": forms.Textarea(attrs={"class": "form-control", "rows": 3, "placeholder": "Description du contrat..."}),
            "notes": forms.Textarea(attrs={"class": "form-control", "rows": 4, "placeholder": "Notes concernant le contrat..."}),
        }
